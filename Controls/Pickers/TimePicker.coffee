...
class exports.TimePicker extends Layer
	...
	createLayers: ->
		...
		for i in [0..timePickerItems.length - 1]
			...
			if i is 0 or i is timePickerItems.length - 1
				hourText.color = minuteText.color = ampmText.color = SystemColor.baseMediumLow
			else if i is 4
				hourText.color = minuteText.color = ampmText.color = SystemColor.baseHigh
				itemContainer.backgroundColor = SystemColor.listAccentLow
			else
				hourText.color = minuteText.color = ampmText.color = SystemColor.baseMedium
	...
	updateVisuals: (curEvent) ->
		if @options.enabled
			timeColor = SystemColor.baseHigh
			...
			timeColor = SystemColor.baseMediumLow
