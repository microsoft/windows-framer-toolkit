...
class exports.Rating extends Layer
	...
	createLayers: ->
		...
		starEmptyColor = UWPColor.baseLow
		starFillColor = UWPColor.accent
		...
		for i in [0..4]
			...
			if i <= @options.starsFilled
				if @options.enabled
					...
					star.color = UWPColor.baseMediumLow
			...
			@container.on Events.MouseMove, (event, sender) ->
				if isTouched
					...
					for i in [0..4]
						...
						if i <= starIndex
							color = if isOff then UWPColor.baseMedium else starFillColor
